...
#
# Ported 2026-09-05 from setup_rl_env.sh (old machine, kept as historical record).
# Only machine-specific bits changed: repo root, conda base, cache/TMPDIR paths,
# MAX_JOBS. The entire pinned stack is IDENTICAL to the old machine:
#
...
#
# CUDA note (new machine): driver 595.71.05 reports "CUDA Version: 13.2" and
# /usr/local only has cuda-13.2 -- this does NOT matter here. The toolkit is the
# self-contained conda-forge cuda-nvcc 12.8 set, torch wheels bundle their own
# cu128 runtime, and driver 595 is backward compatible with everything <= 13.2.
# H200 is sm_90, same compute capability as H100.
#
...
# below (~5-15 min on this 192-core box); the env is usable without it if you set
# use_fused_kernels=False.
#
# Usage:  bash setup_rl_env_h200.sh
...
VERL_DIR="$REPO/verl"
ENV_NAME=gyz_rl
CONDA_BASE=/mnt/beegfs/workspace/scy/miniconda3

# --- caches on beegfs (persistent across rebuilds); TMPDIR on local disk (fast, 738G free) ---
export TMPDIR=/tmp/scy-build
mkdir -p "$TMPDIR" "$REPO/.cache/pip"
export PIP_CACHE_DIR="$REPO/.cache/pip"
export UV_CACHE_DIR=/mnt/beegfs/workspace/scy/.uv-cache
...
# Verified 2026-08-13 (old machine): with 0.40.0 the UNMODIFIED verl run_unvicorn keeps the
# port open (two consecutive GET /health -> 200); with 0.52.1 the same code gets ECONNREFUSED.
pip install --no-input "uvicorn==0.40.0"

echo "===== [optional] flash_attn==2.7.4.post1 (source build; needs nvcc) ====="
...
export CUDA_HOME="$CONDA_PREFIX"
export MAX_JOBS="${MAX_JOBS:-32}"   # 192-core / 2TB box; each job can use a few-to-tens of GB
