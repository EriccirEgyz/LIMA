...
def _create_chat_completion(client, *, model, messages, temperature, max_tokens, enable_thinking, extra_kwargs=None):
    ...
    elif "deepseek" in str(model or "").lower():
        # DeepSeek 官方 thinking 参数控制思考；中转(yibuapi 等)默认思考开启，
        # 必须显式传 disabled 才是真正关掉 —— enable_thinking=False 时不传参数等于默认开。
        request_kwargs["extra_body"] = {"thinking": {"type": "enabled" if enable_thinking else "disabled"}}
    try:
        return client.chat.completions.create(**request_kwargs)
    except Exception as exc:
        error_text = str(exc)
        # 服务端不认识 chat_template_kwargs / thinking 参数时，去掉 extra_body 重试一次
        if "chat_template_kwargs" not in error_text and "thinking" not in error_text:
            raise
...
def openai_stream_inference_fc(
    ...
    while retries < max_retries:
        try:
            ...
            # (deepseek 走官方 thinking 参数，见 _create_chat_completion 里的注入分支)
            effective_thinking = enable_thinking and (model.lower() in _QWEN_THINKING_MODELS or "qwen" in model.lower() or "thinking" in model.lower() or "deepseek" in model.lower())
